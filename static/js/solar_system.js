// solar_system.js - VERSION FINALE NETTOYÉE ET ENRICHIE
...
const planetData = {
    ...
    uranus: { 
        name: "Uranus", distance: 160, diameter: 51118, color: 0x4fd0e7, rotationSpeed: 0.018, orbitSpeed: 0.0011, info: "Une géante de glace de couleur cyan. Sa particularité est d'avoir son axe de rotation incliné à 98°, elle 'roule' donc sur son orbite. Elle possède 27 lunes." 
    },
    neptune: { 
        name: "Neptune", distance: 190, diameter: 49528, color: 0x4b70dd, rotationSpeed: 0.017, orbitSpeed: 0.0009, info: "La planète la plus éloignée du Soleil. Les vents y soufflent à plus de 2 000 km/h, les plus rapides du système solaire. C'est une géante glacée et venteuse possédant 14 lunes." 
    }
};
...
function createControlPanel() {
    ...
    // CORRECTION : Suppression de la case à cocher Rotation Caméra
    panel.innerHTML = `
    ...
    document.getElementById('playPauseBtn').onclick = function() {
        isPaused = !isPaused;
        controls.autoRotate = !isPaused; // On stoppe la rotation camera en pause
...
function addPlanets() {
    Object.keys(planetData).forEach((key) => {
        ...
        // 2. Le Label (Nom) - CORRECTION : Police et Sprite GROSSIS
...
function createLabel(name, radius) {
    ...
    canvas.width = 320; canvas.height = 128; // Canvas plus grand
    // CORRECTION : Police grossie de 32px à 64px
    ctx.font = "Bold 64px Arial";
    ctx.fillStyle = "white";
    ctx.textAlign = "center";
    ctx.fillText(name, 160, 80);
    const sprite = new THREE.Sprite(new THREE.SpriteMaterial({ map: new THREE.CanvasTexture(canvas), transparent: true }));
    // CORRECTION : Sprite grossi de 12x3 à 20x5
    sprite.scale.set(20, 5, 1);
    sprite.position.y = radius + 6; // Ajusté pour ne pas chevaucher la planète
...
function animate() {
    ...
    if (!isPaused) {
        ...
        controls.update(); // Met à jour la rotation camera si pas en pause
    }
...
function addSun() {
    ...
    sun.add(label); sun.userData.label = label;
    scene.add(sun); planets.push(sun);
}
...
function closeModal() { document.getElementById('planet-modal').style.display = 'none'; }

...
def admin_login() -> Union[str, Response]:
    ...
    return render_template(
        "login.html", now=datetime.datetime.now(), title="Admin Login"
    )
...
def admin_dashboard():
    ...
    cur.execute(
        "SELECT id_admin, pseudo, nom, prenom, email FROM ADMINISTRATEUR ORDER BY id_admin ASC"
    )
    ...
    try:
        if new_password:
            if len(new_password) < 6:
                ...
                return redirect(url_for("admin_bp.admin_dashboard") + "#section-admins")
            pwd_hash = _bcrypt.hashpw(
                new_password.encode("utf-8"), _bcrypt.gensalt()
            ).decode("utf-8")
            cur.execute(
                "UPDATE ADMINISTRATEUR SET pseudo=%s, mot_de_passe_hash=%s, "
                "nom=%s, prenom=%s, email=%s WHERE id_admin=%s",
...
def ingest_data():
    ...
    flash(
        f"{count} objets synchronisés !" if count > 0 else "Échec de l'ingestion.",

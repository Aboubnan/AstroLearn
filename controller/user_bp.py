...
from model.database import (
    ...
    check_password,
    create_utilisateur,
    update_utilisateur_profil,
    update_utilisateur_password,
    get_all_categories,
    create_proposition,
    get_propositions_by_user,
    marquer_notifs_lues,
    toggle_favori,
...
def dashboard():
    ...
    stats = {
        ...
        "acceptes": sum(
            1 for p in propositions if p["statut"] in ("accepte", "modifie")
        ),
...
def proposer_objet():
    ...
    if request.method == "POST":
        ...
        if create_proposition(
            ...
            flash(
                "Votre proposition a été envoyée ! L'admin la traitera sous peu.",
                "success",
            )
            return redirect(url_for("user_bp.dashboard"))
        else:
...
def edit_profil():
    ...
    if request.method == "POST":
        ...
        if update_utilisateur_profil(
            session["user_id"], nom, prenom, email, genre, photo_path
        ):

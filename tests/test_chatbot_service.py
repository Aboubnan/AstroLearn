...
@patch("model.chatbot_service.call_mistral_api")
def test_ask_returns_sanitized_response(mock_call_mistral_api):
    mock_call_mistral_api.return_value = (
...
@patch("model.chatbot_service.call_mistral_api")
def test_ask_truncates_overly_long_response(mock_call_mistral_api):
    mock_call_mistral_api.return_value = "x" * (AstroIAChatbot.MAX_RESPONSE_LENGTH + 100)
...
@patch("model.chatbot_service.call_mistral_api")
def test_ask_raises_runtime_error_when_api_returns_nothing(mock_call_mistral_api):
    mock_call_mistral_api.return_value = None
...
@patch("model.chatbot_service.call_mistral_api")
def test_ask_sends_only_last_five_history_messages(mock_call_mistral_api):
    mock_call_mistral_api.return_value = "Réponse"
    history = [{"role": "user", "content": str(i)} for i in range(8)]

    chatbot = AstroIAChatbot(history=history)
    chatbot.ask("Question")

    _, kwargs = mock_call_mistral_api.call_args
